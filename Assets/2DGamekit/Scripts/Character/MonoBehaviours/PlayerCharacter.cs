...
namespace Gamekit2D
{
    ...
    {
        private static PlayerCharacter s_PlayerInstance;
        ...
        {
            // Hacer que el jugador se detenga cuando se agacha
            if (PlayerInput.Instance.Vertical.Value < 0f)
            {
                // Si el personaje esta en el piso, detenerlo
                if (m_CharacterController2D.IsGrounded)
                {
                    velocidadJugador = 0;
                }
                // Sino, no afectar su movimiento
                else
                {
                    velocidadJugador = 5;
                }
